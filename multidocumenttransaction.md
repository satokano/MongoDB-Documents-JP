...
これは、アカウント作成をはじめとする多くの一般的なデータベースオペレーションは、多数の行と列にまたがる原子的な更新が必要だということです。
